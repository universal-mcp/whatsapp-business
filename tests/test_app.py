...
def app_instance():
    ...
    return WhatsappBusinessApp(integration=mock_integration)

def test_universal_mcp_whatsapp_business_app_initialization(app_instance):
    """
    Test that the WhatsappBusinessApp instance is initialized correctly with a name.
    """
...
def test_universal_mcp_whatsapp_business_tool_docstrings_format(app_instance):
    """
    Test that each tool method in WhatsappBusinessApp has a well-formatted docstring,
...
def test_universal_mcp_whatsapp_business_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in WhatsappBusinessApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
